...
function PlanCard({ plan, onReservar }) {
  ...
  return (
    <motion.div
      ...
      style={{
        background: isPopular ? 'linear-gradient(160deg, #1a0a14, #0F172A)' : '#0F172A',
        border: isPopular ? '2px solid rgba(255,0,122,0.45)' : '1px solid #1E293B',
        boxShadow: isPopular ? '0 0 40px rgba(255,0,122,0.15), 0 20px 60px rgba(0,0,0,0.4)' : '0 4px 24px rgba(0,0,0,0.3)',
      }}
      whileHover={{ y: -4, boxShadow: isPopular ? '0 0 60px rgba(255,0,122,0.25)' : '0 8px 40px rgba(255,0,122,0.1)', transition: { duration: 0.25 } }}
    >
      {isPopular && (
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10">
          <span className="flex items-center gap-1.5 bg-gradient-to-r from-[#FF007A] to-[#FF3DAA] text-white text-xs font-bold px-4 py-1.5 rounded-full">
            <Star size={10} fill="white" /> Más popular
          </span>
        </div>
      )}
      <div className="pt-2">
        <h3 className={`font-bold text-lg mb-1 ${isPopular ? 'text-[#FF007A]' : 'text-white'}`}>{plan.nombre}</h3>
        <p className="text-[#9CA3AF] text-sm">{plan.descripcion}</p>
      </div>
      <div className="flex items-end gap-1">
        <span className="text-[#9CA3AF] text-xl font-medium">S/</span>
        <span className="text-white text-5xl font-extrabold tracking-tight leading-none">{plan.precio}</span>
        <span className="text-[#9CA3AF] text-sm mb-1">/ sesión</span>
      </div>
      <div className="text-xs font-medium px-3 py-2 rounded-xl w-fit"
        style={{ background: isPopular ? 'rgba(255,0,122,0.08)' : 'rgba(255,255,255,0.03)', border: `1px solid ${isPopular ? 'rgba(255,0,122,0.2)' : '#1E293B'}`, color: isPopular ? '#FF007A' : '#9CA3AF' }}>
      ...
      <motion.button
        ...
        className={`w-full py-3.5 rounded-full font-semibold text-sm flex items-center justify-center gap-2 transition-all duration-300 ${
          isPopular
            ? 'bg-gradient-to-r from-[#FF007A] to-[#FF3DAA] text-white'
